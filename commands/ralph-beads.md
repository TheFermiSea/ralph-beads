...
bd update $TASK_ID --description "$(cat <<'EOF'
## Acceptance Criteria
- [ ] Criterion 1
- [ ] Criterion 2
EOF
...
## Worktree Cleanup and PR Creation (if --worktree or --pr)

**After molecule completion, if running in worktree:**

```bash
if [ -n "$WORKTREE_PATH" ]; then
  # Save current directory (worktree)
  ORIGINAL_DIR=$(pwd)

  # Return to original repository root
  cd - || cd ..

  # If --pr flag was set, create PR
  if [ "$PR_FLAG" = "true" ]; then
    ...
    # Create PR with molecule info
    EPIC_TITLE=$(bd show <epic-id> | head -1 | cut -d: -f2-)
    gh pr create \
      --title "$EPIC_TITLE" \
      --body "$(cat <<EOF
## Summary
Completed via ralph-beads molecule $MOL_ID
...
---
Generated by ralph-beads with \`--pr\` flag
EOF
)" || {
      echo "WARNING: Failed to create PR - you may need to create manually"
      echo "Branch is pushed: $BRANCH_NAME"
    }
  fi

  # Remove worktree
  git worktree remove "$WORKTREE_PATH" || {
    echo "WARNING: Failed to remove worktree - clean up manually"
...
**Note:** If PR creation fails, the branch is still pushed and you can create the PR manually.
...
bd graph <id>                                    # Dependency visualization
```
